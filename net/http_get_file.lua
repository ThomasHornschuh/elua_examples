...
local t={}
  ...
  -- separate path into to segments
  for w in s:gmatch("/(%w+%.?%w*)") do
    t[#t+1]=w
  end
  if #t>0 then
    return t[#t] -- last element is the filename
  else
    return "noname.html"
  end
end
...
local body
  ...
  repeat
    ...
    if hostip~=0 and net.connect(s,hostip,tonumber(port) or 80)==net.ERR_OK then
      ...
      local fpath="/mmc/"..filename(path)
      f=io.open(fpath,"w+")
      if f then
        f:write(response)
        f:close()
      else
        print(string.format("File %s could not be created",fpath))
      end
